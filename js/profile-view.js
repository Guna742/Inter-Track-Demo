/**
 * InternTrack — Profile View Logic
 * Read-only profile display with scroll reveal animations.
 */
...
(() => {
  ...
  setupSidebar(session, 'profile-view.html');
  ...
  function setupSidebar(session, activePage) {
    ...
    if (roleEl) roleEl.textContent = session.role === 'admin' ? 'Administrator' : 'Intern';

    const items = [
      { label: 'Dashboard', href: 'dashboard.html', icon: '⊞' },
      ...(session.role === 'admin' ? [] : [{ label: 'My Profile', href: 'profile-view.html', icon: '👤' }]),
